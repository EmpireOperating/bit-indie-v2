...
## Worker CLI guardrails (manual runs)
- `--limit` is validated as an integer in range `1..500`.
- Use `--dry-run` first when validating behavior in non-prod/staging environments.

## Collect identifiers (so the SQL below is fast)
Grab **at least one** of these before you start digging:
- `payout_id` (UUID)
- `purchase_id` (UUID)
- `withdrawal_id` (text) — OpenNode withdrawal id
- `invoice_id` (text) — OpenNode invoice/charge id (maps to `Purchase.invoiceId`)

Tip: if you only have the OpenNode side, start with `withdrawal_id` or `invoice_id` and use the context query below to find internal IDs.
...
By provider withdrawal id (join payout → purchase → ledger):
```sql
select count(le.*) as "payoutSentCount"
from "Payout" p
join "LedgerEntry" le on le."purchaseId" = p."purchaseId"
