...
- Invalid radix-literal numeric payloads (`0x...`, `0b...`, `0o...` with malformed digits like `0o89`, plus unknown `0<letter>` prefixes like `0d123`) emit structured warning metadata under `numericMalformedRadixLiteralAnomaly`, including kind tags (`*_malformed_radix_literal_kind`: `hex`/`binary`/`octal`/`unknown`) for faster parse-drift triage beyond generic numeric parse anomalies (behavior-neutral observability only).
