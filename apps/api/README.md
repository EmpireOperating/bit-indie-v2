...
- `confirmed` statuses with `amount < 0` emit structured warning metadata under `confirmedNegativeAmount` for settlement-anomaly triage.
- `confirmed` statuses with `fee == 0` emit structured warning metadata under `confirmedZeroFee` for settlement-anomaly triage.
...
- Invalid radix-literal numeric payloads (`0x...`, `0b...`, `0o...` with malformed digits like `0o89`, bare prefixes like `0x`/`-0b`, plus unknown `0<letter>` prefixes like `0d123`) emit structured warning metadata under `numericMalformedRadixLiteralAnomaly`, including kind tags (`*_malformed_radix_literal_kind`: `hex`/`binary`/`octal`/`unknown`) for faster parse-drift triage beyond generic numeric parse anomalies (behavior-neutral observability only).
...
- `GET /storefront/scaffold/construction/entitlement-telemetry/trace-fixtures` — deterministic cross-lane trace fixtures binding auth events to entitlement consumption telemetry.
- `GET /storefront/scaffold/construction/token-transport/contracts` — headed/headless token transport matrix for entitlement/download execution.
...
- `GET /auth/storefront/construction/runtime/telemetry/payload-templates` — deterministic auth telemetry payload templates for headed/headless runtime emits.
- `GET /auth/storefront/construction/runtime/integration-checks` — executable auth→storefront integration checks for headed QR and headless signed-challenge token handoff.
