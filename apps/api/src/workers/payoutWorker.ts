  ...
  for (const p of payouts) {
    try {
      ...
      if (dryRun) {
        // eslint-disable-next-line no-console
        console.log(
          `[dry-run] would send payout ${p.id} purchaseId=${p.purchaseId} amountMsat=${p.amountMsat} provider=${payoutProvider}`
        );
        continue;
      }
      ...
      let providerMeta: Record<string, any> = { provider: payoutProvider };
