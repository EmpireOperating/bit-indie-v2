...
describe('OpenNode withdrawals webhook', () => {
  ...
  it('logs numeric trailing-dot decimal anomaly for parseable decimal values', async () => {
    const payout: Payout = {
      id: 'pNumericTrailingDot',
      provider: 'opennode',
      providerWithdrawalId: 'wNumericTrailingDot',
      status: 'SUBMITTED',
      amountMsat: '123',
      purchaseId: 'buyNumericTrailingDot',
      providerMetaJson: {},
    };
    ...
    const res = await app.inject({
      ...
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
      payload: new URLSearchParams({
        id: 'wNumericTrailingDot',
        status: 'failed',
        amount: '42.',
        fee: '1.',
        error: 'provider decimal shape drift',
        hashed_order: hmacHex(apiKey, 'wNumericTrailingDot'),
      } as any).toString(),
    });

    expect(res.statusCode).toBe(200);

    const warnLog = parseLogEntries(logs).find((entry) => entry.msg === 'opennode withdrawals webhook: numeric trailing-dot decimal anomaly observed');
    expect(warnLog).toBeTruthy();
    expect(warnLog?.numericTrailingDotAnomaly).toMatchObject({
      withdrawal_id_present: true,
      withdrawal_id_length: 19,
      amount_raw: '42.',
      amount_valid: true,
      amount_has_trailing_dot_decimal_format: true,
      fee_raw: '1.',
      fee_valid: true,
      fee_has_trailing_dot_decimal_format: true,
    });

    const updateArg = (prismaMock.payout.update as any).mock.calls[0][0];
    expect(updateArg.data.providerMetaJson.webhook.amount_has_trailing_dot_decimal_format).toBe(true);
  ...
  it('logs numeric non-finite literal anomaly for NaN/Infinity-like numeric fields non-blockingly', async () => {
    const payout: Payout = {
      id: 'pNumericNonFiniteLiteralAnomaly',
      provider: 'opennode',
      providerWithdrawalId: 'wNumericNonFiniteLiteralAnomaly',
      status: 'SUBMITTED',
      amountMsat: '123',
      purchaseId: 'buyNumericNonFiniteLiteralAnomaly',
      providerMetaJson: {},
    };
    ...
    const prismaMock = {
      ...
      ledgerEntry: {
        findFirst: vi.fn(async () => null),
        create: vi.fn(async () => ({ id: 'leNumericNonFiniteLiteralAnomaly' })),
      },
      $transaction: vi.fn(async (fn: any) =>
        fn({
          payout: prismaMock.payout,
    ...
    const res = await app.inject({
      ...
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
      payload: new URLSearchParams({
        id: 'wNumericNonFiniteLiteralAnomaly',
        status: 'confirmed',
        amount: 'NaN',
        fee: '-Infinity',
        processed_at: '2026-02-08T05:05:00Z',
        hashed_order: hmacHex(apiKey, 'wNumericNonFiniteLiteralAnomaly'),
      } as any).toString(),
    });

    expect(res.statusCode).toBe(200);

    const warnLog = parseLogEntries(logs).find((entry) => entry.msg === 'opennode withdrawals webhook: numeric non-finite literal anomaly observed');
    expect(warnLog).toBeTruthy();
    expect(warnLog?.numericNonFiniteLiteralAnomaly).toMatchObject({
      withdrawal_id_present: true,
      withdrawal_id_length: 31,
      amount_raw: 'NaN',
      amount_valid: false,
  ...
  it('logs numeric malformed radix literal anomaly for invalid 0x/0b inputs non-blockingly', async () => {
    const payout: Payout = {
      id: 'pNumericMalformedRadixLiteralAnomaly',
      provider: 'opennode',
      providerWithdrawalId: 'wNumericMalformedRadixLiteralAnomaly',
      status: 'SUBMITTED',
      amountMsat: '123',
      purchaseId: 'buyNumericMalformedRadixLiteralAnomaly',
      providerMetaJson: {},
    };
    ...
    const prismaMock = {
      ...
      ledgerEntry: {
        findFirst: vi.fn(async () => null),
        create: vi.fn(async () => ({ id: 'leNumericMalformedRadixLiteralAnomaly' })),
      },
      $transaction: vi.fn(async (fn: any) =>
        fn({
          payout: prismaMock.payout,
    ...
    const res = await app.inject({
      ...
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
      payload: new URLSearchParams({
        id: 'wNumericMalformedRadixLiteralAnomaly',
        status: 'confirmed',
        amount: '0xZZ',
        fee: '0b201',
        processed_at: '2026-02-08T05:05:00Z',
        hashed_order: hmacHex(apiKey, 'wNumericMalformedRadixLiteralAnomaly'),
      } as any).toString(),
    });
  ...
  it('classifies malformed octal radix literals for numeric anomaly triage', async () => {
    const payout: Payout = {
      id: 'pNumericMalformedOctalRadixLiteralAnomaly',
      provider: 'opennode',
      providerWithdrawalId: 'wNumericMalformedOctalRadixLiteralAnomaly',
      status: 'SUBMITTED',
      amountMsat: '123',
      purchaseId: 'buyNumericMalformedOctalRadixLiteralAnomaly',
      providerMetaJson: {},
    };
    ...
    const prismaMock = {
      ...
      ledgerEntry: {
        findFirst: vi.fn(async () => null),
        create: vi.fn(async () => ({ id: 'leNumericMalformedOctalRadixLiteralAnomaly' })),
      },
      $transaction: vi.fn(async (fn: any) =>
        fn({
          payout: prismaMock.payout,
    ...
    const res = await app.inject({
      ...
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
      payload: new URLSearchParams({
        id: 'wNumericMalformedOctalRadixLiteralAnomaly',
        status: 'confirmed',
        amount: '0o89',
        fee: '0o18',
        processed_at: '2026-02-08T05:25:00Z',
        hashed_order: hmacHex(apiKey, 'wNumericMalformedOctalRadixLiteralAnomaly'),
      } as any).toString(),
    });
    ...
    expect(warnLog?.numericMalformedRadixLiteralAnomaly).toMatchObject({
      ...
      amount_valid: false,
      amount_looks_malformed_radix_literal: true,
      amount_malformed_radix_literal_kind: 'octal',
      fee_raw: '0o18',
      fee_valid: false,
      fee_looks_malformed_radix_literal: true,
      fee_malformed_radix_literal_kind: 'octal',
    });

    const updateArg = (prismaMock.payout.update as any).mock.calls[0][0];
    expect(updateArg.data.providerMetaJson.webhook.amount_malformed_radix_literal_kind).toBe('octal');
    expect(updateArg.data.providerMetaJson.webhook.fee_malformed_radix_literal_kind).toBe('octal');
  });


  it('classifies malformed signed/uppercase radix literals for numeric anomaly triage', async () => {
    const payout: Payout = {
      id: 'pNumericMalformedSignedUppercaseRadixLiteralAnomaly',
      provider: 'opennode',
      providerWithdrawalId: 'wNumericMalformedSignedUppercaseRadixLiteralAnomaly',
      status: 'SUBMITTED',
      amountMsat: '123',
      purchaseId: 'buyNumericMalformedSignedUppercaseRadixLiteralAnomaly',
      providerMetaJson: {},
    };
    ...
    const prismaMock = {
      ...
      ledgerEntry: {
        findFirst: vi.fn(async () => null),
        create: vi.fn(async () => ({ id: 'leNumericMalformedSignedUppercaseRadixLiteralAnomaly' })),
      },
      $transaction: vi.fn(async (fn: any) =>
        fn({
          payout: prismaMock.payout,
    ...
    const res = await app.inject({
      ...
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
      payload: new URLSearchParams({
        id: 'wNumericMalformedSignedUppercaseRadixLiteralAnomaly',
        status: 'confirmed',
        amount: '+0XGG',
        fee: '-0B210',
        processed_at: '2026-02-08T05:35:00Z',
        hashed_order: hmacHex(apiKey, 'wNumericMalformedSignedUppercaseRadixLiteralAnomaly'),
      } as any).toString(),
    });
  ...
  it('classifies malformed unknown radix-prefix literals for numeric anomaly triage', async () => {
    const payout: Payout = {
      id: 'pNumericMalformedUnknownRadixLiteralAnomaly',
      provider: 'opennode',
      providerWithdrawalId: 'wNumericMalformedUnknownRadixLiteralAnomaly',
      status: 'SUBMITTED',
      amountMsat: '123',
      purchaseId: 'buyNumericMalformedUnknownRadixLiteralAnomaly',
      providerMetaJson: {},
    };
    ...
    const prismaMock = {
      ...
      ledgerEntry: {
        findFirst: vi.fn(async () => null),
        create: vi.fn(async () => ({ id: 'leNumericMalformedUnknownRadixLiteralAnomaly' })),
      },
      $transaction: vi.fn(async (fn: any) =>
        fn({
          payout: prismaMock.payout,
    ...
    const res = await app.inject({
      ...
      headers: { 'content-type': 'application/x-www-form-urlencoded' },
      payload: new URLSearchParams({
        id: 'wNumericMalformedUnknownRadixLiteralAnomaly',
        status: 'confirmed',
        amount: '0d123',
        fee: '-0q9',
        processed_at: '2026-02-08T05:45:00Z',
        hashed_order: hmacHex(apiKey, 'wNumericMalformedUnknownRadixLiteralAnomaly'),
      } as any).toString(),
    });
    ...
    expect(warnLog?.numericMalformedRadixLiteralAnomaly).toMatchObject({
      ...
      amount_valid: false,
      amount_looks_malformed_radix_literal: true,
      amount_malformed_radix_literal_kind: 'unknown',
      fee_raw: '-0q9',
      fee_valid: false,
      fee_looks_malformed_radix_literal: true,
      fee_malformed_radix_literal_kind: 'unknown',
    });
