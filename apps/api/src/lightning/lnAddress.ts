export type LnAddressParts = {
  username: string;
  domain: string;
};

export function parseLnAddress(addr: string): LnAddressParts {
  const trimmed = String(addr ?? '').trim();
  const m = /^([^@\s]+)@([^@\s]+)$/.exec(trimmed);
  if (!m) throw new Error(`Invalid LN address: ${addr}`);
  const username = m[1]!.toLowerCase();
  const domain = m[2]!.toLowerCase();
  if (!username || !domain) throw new Error(`Invalid LN address: ${addr}`);
  return { username, domain };
}

type LnurlPayWellKnown = {
  callback: string;
  tag: string;
  minSendable: number;
  maxSendable: number;
...
function assertHttpUrl(u: string): URL {
  let url: URL;
  try {
    url = new URL(u);
  } catch {
...
export async function fetchLnurlPayParams(lnAddress: string): Promise<LnurlPayWellKnown> {
  ...
  const json = (await res.json()) as Partial<LnurlPayWellKnown>;
  ...
  if (!Number.isFinite(json.minSendable) || !Number.isFinite(json.maxSendable)) {
    throw new Error(`LNURLp invalid min/max for ${lnAddress}`);
  }
  if (json.minSendable <= 0 || json.maxSendable <= 0 || json.minSendable > json.maxSendable) {
    throw new Error(`LNURLp invalid sendable bounds for ${lnAddress}`);
  }

  // Validate callback is a URL; do not allow weird protocols.
  assertHttpUrl(json.callback);

  return json as LnurlPayWellKnown;
}
...
export async function getBolt11InvoiceForLnAddress(opts: {
  ...
  amountMsat: bigint;
  comment?: string;
}): Promise<{ bolt11: string; callback: string }> {
  ...
  if (opts.amountMsat < BigInt(params.minSendable) || opts.amountMsat > BigInt(params.maxSendable)) {
    throw new Error(
      `LNURLp amount out of range for ${opts.lnAddress}: ${opts.amountMsat}msat not in [${params.minSendable}, ${params.maxSendable}]`
    );
  }
  ...
  const json = (await res.json()) as Partial<LnurlPayInvoiceResponse>;
  const errMsg = json.error?.message ?? json.reason;
  if (errMsg) throw new Error(`LNURLp invoice error for ${opts.lnAddress}: ${errMsg}`);
  if (!json.pr) throw new Error(`LNURLp invoice missing pr for ${opts.lnAddress}`);
