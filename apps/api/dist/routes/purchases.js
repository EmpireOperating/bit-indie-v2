...
function parseAmountMsat(v) {
    ...
    // string
    if (!/^[0-9]+$/.test(v))
        throw new Error('amountMsat must be an integer string');
    return BigInt(v);
}
function makeGuestReceiptCode() {
...
export async function registerPurchaseRoutes(app) {
    ...
    app.post('/purchases', async (req, reply) => {
        ...
        const created = await prisma.$transaction(async (tx) => {
            ...
            return { kind: 'ok', purchase, guestReceiptCode };
        });
    ...
    app.post('/webhooks/mock/invoice-paid', async (req, reply) => {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (purchase.status === 'PAID') {
                return { kind: 'ok', purchaseId: purchase.id, already: true };
            }
            ...
            if (!devProfile) {
                return { kind: 'missing_dev_profile', purchaseId: purchase.id };
            }
            ...
            const entitlement = await tx.entitlement.upsert({
                ...
                create: {
                    ...
                    buyerUserId: updatedPurchase.buyerUserId,
            ...
            const { platformFeeMsat, developerNetMsat } = fee10pct(updatedPurchase.amountMsat);
            ...
            const existingTypes = new Set(existingLedger.map((l) => l.type));
            if (!existingTypes.has('INVOICE_PAID')) {
                await tx.ledgerEntry.create({
                    data: {
                        purchaseId: updatedPurchase.id,
                        type: 'INVOICE_PAID',
                        amountMsat: updatedPurchase.amountMsat,
                        metaJson: { paidAt: paidAt.toISOString(), invoiceId: updatedPurchase.invoiceId },
                    },
                });
            }
            ...
            await tx.payout.upsert({
                ...
                create: {
                    ...
                    amountMsat: developerNetMsat,
                    status: 'SCHEDULED',
                    idempotencyKey: `purchase:${updatedPurchase.id}`,
                },
                update: {
                    destinationLnAddress: devProfile.payoutLnAddress,
                    amountMsat: developerNetMsat,
